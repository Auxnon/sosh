defmodule ChatTestWeb.ActionChannelTest do
  use ChatTestWeb.ChannelCase, async: true
  ...
  describe "join" do
    test "joins actions:lobby successfully with authenticated user", %{socket: socket} do
      ...
      {:ok, _, socket} =
        socket
        |> subscribe_and_join(ChatTestWeb.ActionChannel, "actions:lobby")

      assert socket.assigns[:user_id]
    end

    test "rejects join without user_id" do
      socket = Phoenix.ChannelTest.connect(ChatTestWeb.UserSocket, %{}, %{})

      {:error, %{reason: "not_authenticated"}} =
        subscribe_and_join(socket, ChatTestWeb.ActionChannel, "actions:lobby")
    end
  end
